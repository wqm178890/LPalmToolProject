...
+ (NSString *)uniqueID {
    ...
    NSString *ScreenWidthStr = [[[NSString stringWithFormat:@"%d", [SSHardwareInfo screenWidth]] substringToIndex:1] capitalizedString];
    
    // Get the Screen Height
    NSString *ScreenHeightStr = [[[NSString stringWithFormat:@"%d", [SSHardwareInfo screenHeight]] substringToIndex:1] capitalizedString];
    ...
    if (ScreenWidthStr == nil || ScreenWidthStr.length <= 0 || ScreenWidthStr.length > 1 || [ScreenWidthStr isEqualToString:@"-"]) {
        // Invalid String
        ScreenWidthStr = @"1";
    }
    if (ScreenHeightStr == nil || ScreenHeightStr.length <= 0 || ScreenHeightStr.length > 1 || [ScreenHeightStr isEqualToString:@"-"]) {
        // Invalid String
        ScreenHeightStr = @"1";
    }
    ...
    UniqueIdentifier = [NSString stringWithFormat:@"%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@", Salt, DeviceModel, SystemName, DeviceType, ScreenWidthStr, ScreenHeightStr, MultitaskingEnabled, ProximitySensorEnabled, NumberProcessors, ProcessorSpeed, CellMACAddress, WiFiMACAddress, WiFiMACAddress2, WiFiMACAddress3, TotalDiskSpace, Salty];
...
+ (NSString *)deviceSignature {
    ...
    // Get the Screen Height
    NSString *ScreenHeightStr = [NSString stringWithFormat:@"%d", [SSHardwareInfo screenHeight]];
    
    // Get the Screen Width
    NSString *ScreenWidthStr = [NSString stringWithFormat:@"%d", [SSHardwareInfo screenWidth]];
    ...
    if (ScreenHeightStr == nil || ScreenHeightStr.length <= 0 || ScreenHeightStr.length > 4) {
        // Invalid String
        ScreenHeightStr = @"0000";
    }
    if (ScreenWidthStr == nil || ScreenWidthStr.length <= 0 || ScreenWidthStr.length > 4) {
        // Invalid String
        ScreenWidthStr = @"0000";
    }
    ...
    DeviceSignature = [NSString stringWithFormat:@"%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@", SystemVersion, ScreenHeightStr, ScreenWidthStr, PluggedIn, Jailbroken, HeadphonesAttached, BatteryLevel, FullyCharged, ConnectedtoWiFi, DeviceOrientation, Country, TimeZone, NumberProcessors, ProcessorSpeed, TotalDiskSpace, TotalMemory, Salt];
